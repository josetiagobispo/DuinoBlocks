...
$pyjs['loaded_modules']['pyjamas.dnd.utils'] = function (__mod_name__) {
	...
	$m['getComputedStyle'] = function(element, style) {
		...
		var element_style;
		element_style = $p['getattr'](element, 'currentStyle');
		if ($p['bool'](style)) {
	...
	$m['eventCoordinates'] = function(event) {
		var pageY,offsetX,offsetY,$add6,$add7,pageX,$add5,$add8;
		var $tupleassign1 = $p['__ass_unpack']((typeof getScrollOffsets == "undefined"?$m['getScrollOffsets']:getScrollOffsets)(), 2, null);
		offsetX = $tupleassign1[0];
		offsetY = $tupleassign1[1];
		pageX = $p['__op_add']($add5=$p['getattr'](event, 'clientX'),$add6=offsetX);
		pageY = $p['__op_add']($add7=$p['getattr'](event, 'clientY'),$add8=offsetY);
		return $p['tuple']([pageX, pageY]);
	};
	...
	$m['getScrollOffsets'] = function() {
		var scrollOffsetY,scrollOffsetX,$pyjs_try_err;
		try {
			scrollOffsetY = $p['getattr']($p['getattr']($doc, 'body'), 'scrollTop');
			scrollOffsetX = $p['getattr']($p['getattr']($doc, 'body'), 'scrollLeft');
			return $p['tuple']([scrollOffsetX, scrollOffsetY]);
		} catch($pyjs_try_err) {
			var $pyjs_try_err_name = (typeof $pyjs_try_err['__name__'] == 'undefined' ? $pyjs_try_err['name'] : $pyjs_try_err['__name__'] );
			$pyjs['__last_exception__'] = {'error': $pyjs_try_err, 'module': $m};
			if (true) {
				scrollOffsetY = $p['getattr']($p['getattr']($doc, 'documentElement'), 'scrollTop');
				scrollOffsetX = $p['getattr']($p['getattr']($doc, 'documentElement'), 'scrollLeft');
				return $p['tuple']([scrollOffsetX, scrollOffsetY]);
			}
		}
		return null;
	...
	, 1, [null,null,['self']]);
		...
		$method = $pyjs__bind_method2('__str__', function() {
			...
			if ($p['bool']($p['getattr'](self, '_data'))) {
				return $p['__op_add']($add9='\r\n'['join']($p['getattr'](self, '_data')),$add10='\r\n');
			}
			return '';
		}
	, 1, [null,null,['self']]);
